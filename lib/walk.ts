...
import { RESTRICTED_RULES } from "./rules.js";
    ...
    if (node.callee.type === "Identifier" && node.callee.name === RUNE_NAME) {
      if (node.arguments.length !== 1 || node.arguments[0].type !== "Literal") {
        throw buildError(
          node,
          "Invalid $css call",
          "$css must have exactly one argument"
        );
      }
      ...
      Attribute: (node, { next, state }) => {
        if (node.name === "class") {
          if (
            ...
            values.forEach((val) =>
              usedClasses.set(val, { start: node.start, end: node.end })
            );
            next({ inClass: false });
          } else {
            next({ inClass: true });
      ...
      ObjectExpression: (node, { next, state }) => {
        node.properties.forEach((prop) => {
          if (prop.type === "Property") {
            const key = prop.key;
            if (key.type === "Literal") {
              usedClasses.set(key.value?.toString() || "", {
                start: (node as any).start,
                end: (node as any).end,
              });
            }
            if (key.type === "Identifier") {
              usedClasses.set(key.name, {
                start: (node as any).start,
                end: (node as any).end,
              });
            }
 ...
 */
const groupChained = <T extends { chain?: number; start: number; end: number }>(
  selectors: T[]
): T[][] => {
  const groups: T[][] = [];
  let current: T[] = [];
...
const groupPermutations = (
  ...
  selectors.forEach((selector, idx) => {
    ...
    if (
      selector.chain === currentChain &&
      selector.start == lastChar &&
      permutations[idx]
    ) {
      current.push(selector);
    } else {
 ...
 */
const validateClassPlacement = (
  otherSelectors: AST.BaseNode[],
  runeClasses: SvelteFragments["ClassSelector"][],
  nativeClasses: Map<string, { start: number; end: number }>,
  source: string,
  start: number,
  end: number
) => {
  const error = (cause: PlacedClass) => {
    const e = new Error(
      "Invalid class placement. Svelte only allows global classes at the beginning or end of a selector list."
    );
    (e as any).start = start;
    (e as any).end = end;
    (
      e as any
    ).detail = `Contains a selector that is used by runes and is not in the beginning or end of the selector list. \n\n`;
    if (cause.kind === "mixed") {
      (
        e as any
      ).detail += `The class "${cause.name}" is used with the $css rune and natively. It can only be used the first or last selector.\n`;
      (
        e as any
      ).detail += `Consider using the $css rune for all references to this class "${cause.name}".`;
    }
    if (cause.kind === "rune") {
      (
        e as any
      ).detail += `The class "${cause.name}" is used with the $css rune. Other selectors in this rule are used natively or mixed. Classes that are used with the $css rune must be in the beginning or end of the selector.`;
    ...
    (
      e as any
    ).detail += `For more information see: https://github.com/JanNitschke/svelte-css-rune#edge-cases`;

    throw e;
  };

  const selectors: PlacedClass[] = otherSelectors.map((node) => ({
    name: (node as any).name,
    start: node.start,
    end: node.end,
    kind: "native",
  }));
  runeClasses.forEach((node) =>
    selectors.push({
      name: node.name,
      start: node.start,
      end: node.end,
      kind: nativeClasses.has(node.name) ? "mixed" : "rune",
    })
  );
  ...
  let lastEnd = selectors[0].start;
  let cause: PlacedClass | null = null;

  for (let i = 0; i < selectors.length; i++) {
    // Reset if we are in a chain
    if (!RESTRICTED_RULES) {
      const textBetween = source.substring(lastEnd, selectors[i].start);
      if (textBetween.includes(",")) {
        isInStart = true;
        isInEnd = false;
      }
      lastEnd = selectors[i].end;
    }
...
const warnMixedUsage = (
  ...
  warning += "\n\n" + selLoc.text;
  warning += printBelow(
    "combines selectors that are used with and without the $css rune",
    selLoc.startColumn
  );
  if (runeUsed.length > 1) {
    warning += `\n\nThe classes ${runeUsed.join(
      ", "
    )} are used with the $css rune.`;
  } else {
  ...
  if (runeNotUsed.length > 1) {
    warning += `\nThe selectors ${runeNotUsed.join(
      ", "
    )} are not used with the $css rune.`;
  } else {
    warning += `\nThe selector ${runeNotUsed[0]} is not used with the $css rune.`;
  }
  warning +=
    'You can suppress this warning by setting the `mixedUseWarnings` to `false` or `"use"`.\n';
  warning +=
      ...
      Rule: (node, { next, state }) => {
        ...
        if (ruleChangedClasses.length > 0) {
          ...
          if (ruleChangedClasses.some((val) => usedClasses.has(val.name))) {
            ...
            const selectorString = source.substring(
              selectors.start,
              selectors.end
            );
            ...
            const groups = groupChained(
              ruleChangedClasses as (SvelteFragments["ClassSelector"] & {
                chain: undefined;
              })[]
            );
            groups.forEach((group) => {
          ...
          if (emitWarnings) {
            if (ruleUnchangedSelectors.length > 0) {
              warnMixedUsage(
                ...
                fileName,
                fileContent,
                ruleChangedClasses.map((val) => val.name),
                ruleUnchangedSelectors.map((val) => (val as any).name)
              );
            }
          }
        }
