...
import { RESTRICTED_RULES } from "../lib/rules.ts";

const hash = () => "hash";

const transform = (content, warn = false) =>
  processCssRune({ hash, mixedUseWarnings: warn }).markup({
    content,
  ...
  if (!RESTRICTED_RULES) {
    it("should allow multiple selectors", () => {
      const { document, getStyle } = toDOM(`
        <script> const t = ''; </script>
        <span class="{$css("container test")} local {$css("mixed")}">test</span>
        <span class="{$css("container test2")} local {$css("mixed")}">test</span>
        <style>.container.test.local.mixed, .container.test2.local.mixed{ color: rebeccapurple }</style>
        `);
      const span = document.querySelector("span");
      expect(getStyle(span)?.color).toEqual("rebeccapurple");
    });
