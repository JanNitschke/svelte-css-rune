...
import { Window, type CSSStyleDeclaration, type Element } from "happy-dom";
...
const transformEnv = (
  options: Partial<Options & { env?: string[] }>,
  load: boolean = true
) => {
 ...
 */
export const toCSSOnly = (
  code: string,
  options: Partial<Options & { env?: string[] }> = DEFAULT_OPTIONS
) => {
  const combinedOptions = { ...DEFAULT_OPTIONS, ...options };
  const { css } = options.env
    ? transformEnv(combinedOptions, false)
    : { css: "" };
  ...
  return {
    document,
    getStyle: (node?: Element | null | undefined): CSSStyleDeclaration | null =>
      !node || !(node as any).style
        ? null
        : window.getComputedStyle(node as any),
  };
};
...
export const toDOM = (
  ...
  const { env, css } = options.env
    ? transformEnv(combinedOptions, true)
    : { env: {}, css: "" };
  ...
  const module = importFromStringSync(compiled.js.code, {
    globals: { env, console, send },
  });
